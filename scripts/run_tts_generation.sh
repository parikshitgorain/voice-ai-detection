...
run_lang() {
  ...
  local out_dir="${OUT_BASE}/${lang}/generated"
  if [ ! -f "$text_file" ]; then
    log "TTS generation skipped: ${lang} text file missing (${text_file})"
    return 0
  fi
  log "TTS generation start: ${lang} (5000 samples)"
  # Limit to 4 cores to avoid starving training
  taskset -c 0-3 nice -n 10 "$VENV_PY" "$GEN" \
    --text "$text_file" \
    --out "$out_dir" \
    --lang "$lang" \
    --max 5000 >> "/tmp/tts_${lang}.log" 2>&1
  log "TTS generation complete: ${lang}";
}
